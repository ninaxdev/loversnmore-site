            ...
            if (__isEmpty($profileVisitorData)) {
                ...
                if ($this->userRepository->storeProfileVisitors($storeData)) {
                    ...
                    if (!$allowVisitorProfile) {
                        ...
                        $notificationData=[
                            'message'=> 'Profile visited by' . ' ' . $loggedInUserName,      //Message request received from,
                             'action'=> route('user.profile_view', ['username' => $loggedInUser->username]), 
                             'isRead'=> null,
                              'userId'=> $userId,
                              'type'=> 1,//new visitor
...
//if it is empty
            ...
            if ($this->userRepository->updateLikeDislike($likeDislikeData, $updateData)) {
                ...
                if ($like == 1) {
                    ...
                    activityLog($userFullName . ' ' . 'profile liked.');
                          ...
                          //notification log message
                          $notificationData=[
                            'message'=>  'Profile liked by' . ' ' . $loggedInUserFullName,      //like user,
            ...
            if ($this->userRepository->storeLikeDislike($storeData)) {
                ...
                if ($like == 1) {
                    ...
                    if ($showLikeNotification) {
                        //notification log message
                        $notificationData=[
                            'message'=> 'Profile liked by'. ' ' . $loggedInUserFullName, //profile like,
                             'action'=> route('user.profile_view', ['username' => $loggedInUserName]), 
                             'isRead'=> null,
                              'userId'=> $user->_id,
                              'type'=> 2,//user like
            ...
            if (__isEmpty($profileVisitorData)) {
                ...
                if ($this->userRepository->storeProfileVisitors($storeData)) {
                    ...
                    if (!$allowVisitorProfile) {
                        //notification log message
                        $notificationData=[
                            'message'=> 'Profile visited by' . ' ' . $loggedInUserName,  //profile visited
                             'action'=> route('user.profile_view', ['username' => $loggedInUser->username]), 
                             'isRead'=> null,
                              'userId'=> $userId,
                              'type'=> 1,//new visitor
    ...
    public function userWelcomeNotifyMail($newUser)
    {
        ...
        $welcomeEmailSubject =  __tr("Welcome To ") . getStoreSettings('name');
    ...
    public function verifyOtpProcess($inputData)
    {
        ...
        if (Session::has('sms_otp')) {
            ...
            return $this->engineReaction(1, ['show_message' => true], __tr('Verification failed.'));
        }

        return $this->engineReaction(2, ['show_message' => true], __tr('Something went wrong.'));
    }
     /**
     * Api Verify Otp Process
     *
     * @param   array  $inputData
     *
     * @return  json object
     */
    public function processPasswordResetViaApp($inputData)
    {
                ...
                if ($user->remember_token == $inputData['otp']) {
                     // Check if user password updated
                  if ($this->userRepository->resetPassword($user, $inputData['password'])) {
                  return $this->engineReaction(1, ['show_message' => true, 'password_changed' => true,], __tr('Password reset successfully.'));
                    }
                  //failed response
                   return $this->engineReaction(2, ['show_message' => true], __tr('Password not updated.'));
