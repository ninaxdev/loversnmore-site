...
<div x-data="{
    ...
    openSections: {
        age: <?= !__isEmpty($request->min_age) || !__isEmpty($request->max_age) ? 'true' : 'false' ?>,
        ...
        height: <?= !__isEmpty($request->min_height) || !__isEmpty($request->max_height) ? 'true' : 'false' ?>,
    },
    ...
    <div class="lw-card-glass">
        <div class="p-4 md:p-6">
            <form class="lw-ajax-form lw-action-with-url" method="get" data-show-processing="true" action="<?= route('user.read.find_matches') ?>">
                ...
                <div class="mb-4 border-2 rounded-lg overflow-hidden" style="border-color: #ec9cae;">
                    <button type="button"
                        ...
                        <div class="flex items-center gap-3">
                            <span class="text-sm text-gray-600" x-text="min_age + ' - ' + max_age"></span>
                            <i class="fas fa-chevron-down transition-transform duration-200"
                               :class="openSections.age ? 'rotate-180' : ''"></i>
                        </div>
                    </button>
                    <div x-show="openSections.age"
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="min_age" x-model="min_age" class="lw-form-select w-full">
                            ...
                            <div>
                                ...
                                <select name="max_age" x-model="max_age" class="lw-form-select w-full">
                                    @foreach(configItem('user_settings.age_range') as $age)
                                    <option value="<?= $age ?>"><?= $age ?></option>
                                    @endforeach
                                </select>
                ...
                <!-- Height Collapsible -->
                @if(isset($userSpecifications['groups']['basics']['items']['height']))
                <?php $heightItem = $userSpecifications['groups']['basics']['items']['height']; ?>
                <div class="mb-4 border-2 rounded-lg overflow-hidden" style="border-color: #ec9cae;">
                    <button type="button"
                            @click="toggleSection('height')"
                            class="w-full px-4 py-4 flex items-center justify-between bg-white hover:bg-gray-50 transition-colors">
                        <span class="font-semibold lw-font" style="color: var(--lw-primary);">
                            <?= __tr('Height') ?>
                        </span>
                        <i class="fas fa-chevron-down transition-transform duration-200"
                           :class="openSections.height ? 'rotate-180' : ''"></i>
                    </button>
                    <div x-show="openSections.height"
                         x-transition:enter="transition ease-out duration-200"
                         x-transition:enter-start="opacity-0 transform -translate-y-2"
                         x-transition:enter-end="opacity-100 transform translate-y-0"
                         class="px-4 py-4 bg-gray-50 border-t-2" style="border-color: #ec9cae;">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="min_height" class="lw-form-select w-full">
                                    ...
                                    @if(isset($heightItem['options']))
                                    @foreach($heightItem['options'] as $optionKey => $option)
                ...
                <div class="mb-4 border-2 rounded-lg overflow-hidden" style="border-color: #ec9cae;" x-data="{ open: false }">
                    ...
                    <div x-show="open"
                        ...
                        @if($item['input_type'] == 'select' && $itemKey != 'height')
